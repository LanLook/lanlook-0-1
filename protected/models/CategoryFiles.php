 ...
 *
 * The followings are the available columns in table 'category_files':
 * @property integer $id
 ...
 */
class CategoryFiles extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return CategoryFiles the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			...
			array('category_id, file_status', 'numerical', 'integerOnly'=>true),
			array('file_name, file_description', 'length', 'max'=>200),
			array('file_content', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, category_id, file_name, file_content, file_description, created_date, modified_date, file_status', 'safe', 'on'=>'search'),
		);
	}
